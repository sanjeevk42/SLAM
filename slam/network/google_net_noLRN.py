...
class GoogleNet(Network):
    ...
    def add_loss(self, groundtruth):
        ...
        loss1, loss2, loss3 = self.get_loss(output1, groundtruth), self.get_loss(output2, groundtruth), self.get_loss(
            output3, groundtruth)
    ...
    def add_l2_loss(self, groundtruth):
        output1, output2, output3 = self.layers['output1'], self.layers['output2'], self.layers['output3']
        loss1, loss2, loss3 = self.get_l2_loss(output1, groundtruth), self.get_l2_loss(output2,
                                                                                       groundtruth), self.get_l2_loss(
            output3, groundtruth)
        tf.scalar_summary('loss1', loss1)
        tf.scalar_summary('loss2', loss2)
        tf.scalar_summary('loss3', loss3)
