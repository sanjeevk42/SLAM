...
class VGG16Model:
    ...
    def build_graph(self):
        ...
        with tf.variable_scope('vgg16'):
            ...
            with tf.variable_scope('conv1'):
                conv1_1 = self.add_conv_layer('conv1_1', self.network_input, filter_size, input_channels, 64,
                                              should_init_wb=False)
            ...
            with tf.variable_scope('out_layer'):
                out_weights = self.__get_variable('weights', [4096, self.output_dim], tf.float32,
                                                  initializer=tf.truncated_normal_initializer(stddev=5e-2))
                out_bias = self.__get_variable('bias', [self.output_dim], tf.float32,
                                               initializer=tf.truncated_normal_initializer(stddev=5e-2))
    ...
    def __get_init_params(self, scope_name, should_init_wb):
        if scope_name in self.initial_params and should_init_wb:
            ...
            self.logger.warn(
                'No initial weights found for scope:{}. Initializing with random weights.'.format(scope_name))
